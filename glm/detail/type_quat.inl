...
#		endif
	...
	//	vec<3, valType> camera = glm::cos(eulerAngle * valType(0.5));
	//	vec<3, valType> s = glm::sin(eulerAngle * valType(0.5));
	//
	//	this->w = camera.x * camera.y * camera.z + s.x * s.y * s.z;
	//	this->x = s.x * camera.y * camera.z - camera.x * s.y * s.z;
	//	this->y = camera.x * s.y * camera.z + s.x * camera.y * s.z;
	//	this->z = camera.x * camera.y * s.z - s.x * s.y * camera.z;
	//}
